...
app = Flask(__name__)
app.secret_key = 'yousef_ali_2026_secure'
...
def register():
    ...
    if not is_password_strong(data.get('password')):
        return jsonify({"status": "error", "message": "Password weak! Use 8+ chars (A-Z, a-z, 0-9)"}), 400
...
def analyze():
    ...
    fname = request.json.get('filename', 'Unknown.json')
    new_scan = ScanHistory(filename=fname, result=res_status, confidence=conf, user_id=session['user_id'])
    db.session.add(new_scan)
    db.session.commit()
    return jsonify({"status": res_status, "confidence": conf})
